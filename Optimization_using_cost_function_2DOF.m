...
damping_vector = linspace(250,10000,30);     % Define a dampingvector with 30 values from the lower to the upper bound

cost = 0;      % Initialize the cost variable
k = 0;     % Initialize the counting variable for the rows of the cost matrix
for i=1:length(damping_vector) % Define a for loop which loops over every damping vector entry for the front suspension
    k = k+1;                       % First counting variable goes one up
    kk = 0;                         % Initialize the counting variable for the columns of the cost matrix
    for j=1:length(damping_vector) % Define a for loop which loops over every damping vector entry for the rear suspension
        kk = kk+1;                % Second counting variable goes one up
        cost(k,kk) = Exercise_4_4_parameter_fit_obj_local(damping_vector(k),damping_vector(kk));  %#ok<SAGROW> % Define the objective function which gives you the cost value for a given damping constant combination. 
    end
end
[X,Y] = meshgrid(damping_vector);   % Use the command meshgrid to generate a matrix which gives you a combination of each damping value 
surf(X,Y,cost)      % Plot the cost matrix using the surf command
xlabel('Damping Front')  % Label the x axis, take a moment to think about the label is it rear or front damping?
ylabel('Damping Rear') % Label the y axis, take a moment to think about the label is it rear or front damping?
...
[damping_opt,value] = fminsearch(f,[250;10000])   % Find the local minimum by using "fminsearch" notice that fmincon would also work but is not support in the edX gui yet.
%[min_cost column] = min(min(cost));
%[min_cost row] = min(cost(:,column));
%Damper_FR=damping_vector(row)
%Damper_RR=damping_vector(column)
...
cost = abs_motion_displacement*0.4+abs_motion_rotation*0.6; % Give a weighing factor of 0.4 to the displacement and 0.6 to the pitch motion
