...
force = mass_extruded*radius*omega^2;                                       % Calculate the force with given parameters
...
x_max = force/sqrt((damping*omega)^2+(stiffness-mass*omega^2)^2);          % maximum amplitude of particular solution
phi = atan((damping*omega)/(stiffness-mass*omega^2));                                         % phase shift of the particular solution

k1 = x_0-x_max*cos(-phi)+(-lambda(1)*x_0+x_dot_0+x_max*omega*sin(-phi)+x_max*cos(-phi)*lambda(1))/(lambda(1)-lambda(2));% Calculation of the constant 1
k2 = (lambda(1)*x_0-x_dot_0-x_max*omega*sin(-phi)-x_max*cos(-phi)*lambda(1))/(lambda(1)-lambda(2));                                          % Calculation of the constant 2
